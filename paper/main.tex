...
We further show that spectral graph properties---particularly algebraic connectivity, spectral gap, and clustering coefficient---are the strongest predictors of GNN failure, yet the prediction task itself is at chance: balanced accuracy 0.52 and F1 0.12, with the 88\% raw accuracy no better than the 98.2\% majority-class baseline.
...
\begin{enumerate}
    ...
    \item \textbf{Failure Prediction}: We train classifiers on spectral graph properties to predict when GNNs fail. The task is at chance---balanced accuracy 0.52, F1 0.12---because the GNN wins too rarely (1.8\%) to learn a useful predictor; the 88\% raw accuracy is no better than the 98.2\% majority-class baseline.
\end{enumerate}
...
\midrule
Greedy (random) & 1.283 & 0.0\% & $< 0.001$ \\
Welsh-Powell & 1.138 & 0.0\% & $< 0.001$ \\
DSatur & \textbf{1.003} & \textbf{98.8\%} & 0.005 \\
\textbf{GNN + repair} & 3.396 & 0.0\% & 0.005 \\
\bottomrule
\end{tabular}
\end{table}

Graph coloring is the GNN's worst problem: it uses $3.4\times$ more colors than DSatur on average, and wins 0\% of 600 instances across all families and sizes.
...
\textbf{Key findings:}
\begin{itemize}
    \item The minimum eigenvalue of the adjacency matrix ($\lambda_{\min}(A)$) is the single strongest predictor of classical dominance.
    \item Graphs with high clustering coefficients and low spectral gap (e.g., Watts-Strogatz small-world) are hardest for GNNs.
