...
For Max-Cut, the Goemans-Williamson SDP relaxation ($\alpha \geq 0.878$) dominates the GNN across all graph families, with the GNN winning only 2.2\% of 600 instances.
...
We further show that spectral graph properties---particularly algebraic connectivity, spectral gap, and clustering coefficient---predict GNN failure modes with 88\% accuracy.
...
\begin{enumerate}
    ...
    \item \textbf{Failure Prediction}: We train classifiers on spectral graph properties to predict when GNNs fail, achieving 88\% accuracy on Max-Cut.
\end{enumerate}
...
We use the Graph Isomorphism Network (GIN) \cite{xu2019powerful} throughout, as it is provably the most expressive standard message-passing GNN.
...
\begin{table}[h]
\centering
\caption{Max-Cut results across 600 instances (5 families $\times$ 4 sizes $\times$ 30 instances). GNN includes local search refinement.}
\label{tab:maxcut}
\begin{tabular}{lcccc}
\toprule
...
Greedy & $\geq 0.5$ & 0.945 & 5.3\% & 0.003 \\
Spectral & --- & 0.978 & 25.8\% & 0.003 \\
Goemans-Williamson & $\geq 0.878$ & \textbf{0.999} & \textbf{91.5\%} & 1.171 \\
\textbf{GNN + local search} & --- & 0.956 & 8.5\% & 0.053 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{TSP results on Euclidean instances (3 sizes $\times$ 30 instances = 90 total).}
\label{tab:tsp}
\begin{tabular}{lcccc}
\toprule
\textbf{Algorithm} & \textbf{Guarantee} & \textbf{Avg Length/Best} & \textbf{Win Rate} & \textbf{Avg Time (s)} \\
\midrule
Nearest Neighbor & $O(\log n)$ & 1.153 & 1.1\% & $< 0.001$ \\
...
The most striking TSP finding is that GNN+2-opt produces \emph{identical} tour lengths to NN+2-opt.
...
\begin{table}[h]
\centering
\caption{Graph Coloring results across 600 instances.}
\label{tab:coloring}
\begin{tabular}{lccc}
\toprule
...
DSatur & \textbf{1.003} & \textbf{100\%} & 0.005 \\
...
\end{subfigure}
\caption{Failure prediction. Spectral features (minimum adjacency eigenvalue, clustering coefficient, spectral gap) are the strongest predictors of GNN failure. Logistic regression achieves 88\% accuracy.}
\label{fig:failure}
\end{figure}
...
We also demonstrated that spectral graph properties can predict GNN failure modes with high accuracy.
...
For practitioners, the takeaway is clear: benchmark against the best known algorithms, not just baselines.
